...
- anchor profile: enumerate domains via `SELECT DISTINCT host FROM articles`, `load_profile(main_domain)` each, `group_by_target_url`, then **re-key each bucket through `canonicalize_url`** to match the join. Missing profile → no anchor data (distinct from 0.0); pre-bump `""` rollup bucket → flag `(domain-level)`.
