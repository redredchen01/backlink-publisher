...
``telegra.ph`` allows an operator to create a public post directly from a
browser page without a conventional login ceremony.  This module groups those
channels under the same implementation style: launch (or attach to) a real
Chrome DevTools session, fill the public composer, publish, and return the
URL now visible in the browser.
"""
